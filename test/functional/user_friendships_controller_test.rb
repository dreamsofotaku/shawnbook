...
class UserFriendshipsControllerTest < ActionController::TestCase
	context "#index" do
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:taylorjones), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:taylorjones), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
				@friendship3 = create(:requested_user_friendship, user: users(:taylorjones), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
				@friendship4 = user_friendships(:blocked_by_taylor)

				sign_in users(:taylorjones)
				get :index
			end
	...
	context "#new" do
		...
		context "when logged in" do
			...
			should "display the friend's name" do
				get :new, friend_id: users(:taylorjones)
				assert_match /#{users(:taylorjones).full_name}/, response.body
			end
			...
			should "assign a new user friendship to the correct friend" do
				get :new, friend_id: users(:taylorjones)
				assert_equal users(:taylorjones), assigns(:user_friendship).friend
			end

			should "assign a new user friendship to the currently logged in user" do
				get :new, friend_id: users(:prince)
				assert_equal users(:taylorjones), assigns(:user_friendship).user
			end
			...
			should "ask if you really want to friend the user" do
				get :new, friend_id: users(:taylorjones)
				assert_match /Do you really want to friend #{users(:taylorjones).full_name}?/, response.body
			end
		end
	end
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:pending_user_friendship, user: users(:taylorjones), friend: @friend)
				create(:pending_user_friendship, friend: users(:taylorjones), user: @friend)
				sign_in users(:taylorjones)
	...
	context "#edit" do
		...
		context "when logged in" do
			setup do
				@user_friendship = create(:pending_user_friendship, user: users(:taylorjones))
				sign_in users(:taylorjones)
	...
	context "#destroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:taylorjones))
				create(:accepted_user_friendship, friend: users(:taylorjones), user: @friend)
			...
			should "set the flash" do
				delete :destroy, id: @user_friendship
				assert_equal "You are no longer friends with #{@user_friendship.friend.full_name}.", flash[:success]
			end
		end
	end
