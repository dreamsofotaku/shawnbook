...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should not display blocked user's post when logging in" do
    sign_in users(:taylorjones)
    users(:blocked_friend).statuses.create(content: 'Blocked status')
    users(:prince).statuses.create(content: 'Non-blocked status')
    get :index
  ...
  test "should create status for current user when logged in" do
    sign_in users(:taylorjones)
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:prince).id }
    end

    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:taylorjones).id
  end
  ...
  test "should update status for current user when logged in" do
    sign_in users(:taylorjones)
    put :update, id: @status, status: { content: @status.content, user_id: users(:prince).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:taylorjones).id
  end
